...
// DOM-based Editor Engine - orchestrates all editor components
// Drop-in replacement for canvas-based Engine
...
export class DOMEngine {
  constructor(frontContainerEl, backContainerEl, options = {}) {
    ...
    this.canvases = new DOMCanvasPair(frontContainerEl, backContainerEl, {
      qualityScale: options.qualityScale || 3
    })

    // Selection overlays for each side
    this.selectionFront = new DOMSelection(frontContainerEl)
    this.selectionBack = new DOMSelection(backContainerEl)
  ...
  async start() {
    ...
    if (frontBackground || backBackground) {
      try {
        ...
        console.error('[DOMEngine] failed to load backgrounds:', err)
      }
    } else {
