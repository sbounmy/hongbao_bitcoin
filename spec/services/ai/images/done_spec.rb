...
RSpec.describe Ai::Images::Done, type: :service do
  ...
  describe '#call', vcr: { cassette_name: "ai/images/done_success" }  do
    ...
    it 'attaches the images to the image' do
      expect { subject }.to change { ai_images(:christmas_bill).reload.images.count }.by(1)
    end

    it 'creates papers' do
      expect { subject }.to change { Paper.count }.by(1)
      paper = Paper.last
      expect(paper.public).to be false
      expect(paper.active).to be true
    end
  end
end
